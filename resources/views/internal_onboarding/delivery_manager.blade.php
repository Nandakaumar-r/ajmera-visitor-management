...
<div class="container mt-5">
    <h2 class="mb-4 text-center fw-bold" style="margin-top: 90px;">Delivery Manager Review</h2>
...
<div class="d-flex justify-content-between mt-4 align-items-center">
    <a href="{{ url('/orf/' . $role) }}" class="btn btn-secondary" style="height: 40px;">← Back to List</a>

    @php
        $currentStatus = $orf->{$role . '_status'};
        $currentRemarks = $orf->{$role . '_remarks'};
        ...
        $nextRole = $roleFlow[$role] ?? null;
    @endphp

    @if ($currentStatus === null)
        {{-- Show Approve/Reject Buttons --}}
        <div class="d-flex">
            <form action="{{ route('orf.approve', ['role' => $role, 'id' => $orf->id]) }}" method="POST" class="me-2">
                @csrf
                <input type="hidden" name="comments" value="Approved by {{ strtoupper($role) }}">
                <button type="submit" class="btn btn-success">Approve</button>
            </form>

            <form action="{{ route('orf.reject', ['role' => $role, 'id' => $orf->id]) }}" method="POST" class="d-flex">
                @csrf
                <input type="text" name="comments" class="form-control me-2" placeholder="Rejection reason" required>
                <button type="submit" class="btn btn-danger">Reject</button>
            </form>
        </div>
    @elseif ($currentStatus === 'approved')
        <div class="alert alert-success mb-0">
            Approved by {{ strtoupper($role) }} and forwarded to {{ strtoupper($nextRole) }}.
        </div>
    @elseif ($currentStatus === 'rejected')
        <div class="alert alert-danger mb-0">
            Rejected by {{ strtoupper($role) }}<br>
            <strong>Reason:</strong> {{ $currentRemarks }}
        </div>
    @endif
</div>
    </div>
