        ...
        <div class="p-4 sm:p-6">
            ...
            <div class="mt-4 flex items-center space-x-4">
                <button onclick="toggleLike({{ $post->id }})" 
                        class="like-button flex items-center text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 {{ $post->likedBy(auth()->user()) ? 'text-indigo-600 dark:text-indigo-400' : '' }}"
                        data-post-id="{{ $post->id }}">
                    <svg class="h-5 w-5" fill="{{ $post->likedBy(auth()->user()) ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>
                    </svg>
                    <span class="ml-2 likes-count">{{ $post->likes_count }}</span>
                </button>
                <button onclick="toggleComments({{ $post->id }})" 
                        class="comment-button flex items-center text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400"
                        data-post-id="{{ $post->id }}">
