...
<div style="max-width: 1200px; margin: 4rem auto; margin-top: 100px; padding: 2rem; background: #fff; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.1); font-family: Arial, sans-serif; color: #333;">
    ...
    @php
    $expenses = json_decode($reimbursement->details, true);
    @endphp
    ...
    <table style="width: 100%; border-collapse: collapse; margin-bottom: 2rem; font-size: 15px; background: #fafafa;">
        <tbody>
            ...
            $currency = 'Multiple';
             ...
             $totalAmountDisplay = $currency . ' ' . number_format($reimbursement->amount, 2);
            @endphp
            @foreach([
            'Name' => $reimbursement->user->name ?? 'N/A',
            'Manager' => $reimbursement->user->employee->manager->manager_name ?? 'N/A',
            'Emp ID' => $reimbursement->user->employee->employee_id ?? 'N/A',
            'Company' => $reimbursement->company ?? 'N/A',
            'Total Amount' => $totalAmountDisplay,
            'Status' => $reimbursement->status,
            'Remarks' => $reimbursement->rejection_reason ?? 'N/A'
            ] as $label => $value)
    ...
    <div style="margin-bottom: 15px;">
        <a href="{{ route('internal-reimbursement.download.bills', $reimbursement->id) }}" style="margin-right: 10px; padding: 8px 14px; background-color: #ffc107; color: black; border-radius: 6px; text-decoration: none;">📥 Download All Bills</a>
    </div>


    {{-- Approval Form --}}
    <form action="{{ route('internal_handle', ['id' => $reimbursement->id]) }}" method="POST" onsubmit="return validateForm(event)">
        @csrf
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <textarea id="rejection_reason" name="rejection_reason" rows="4" style="width: 100%; padding: 12px; border: 1px solid #ccc; border-radius: 8px; resize: vertical;">{{ old('remarks', $reimbursement->rejection_reason ?? '') }}</textarea>
            @error('remarks')
