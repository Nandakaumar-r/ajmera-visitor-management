<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>External Reimbursement Rejected</title>
</head>
<body style="margin: 0; padding: 10px; font-family: Arial, sans-serif; background-color: #f6f6f6;">
 
    <table width="100%" cellpadding="0" cellspacing="0" style="padding: 40px 0; background-color: #f6f6f6;">
        <tr>
            <td align="center">
                <table width="600" cellpadding="0" cellspacing="0" style="background-color: #fff; border-radius: 8px; box-shadow: 0 4px 10px rgba(0,0,0,0.1);">
                   
                    <!-- Logo -->
                    <tr>
                        <td style="text-align: center; padding: 20px;">
                            <img src="{{ url('images/logo.png') }}" alt="Company Logo" style="height: 60px;">
                        </td>
                    </tr>
 
                    <!-- Heading -->
                    <tr>
                        <td style="text-align: center; padding: 20px 30px;">
                            <h2 style="color: #d9534f; margin: 0;">External Reimbursement Rejected</h2>
                        </td>
                    </tr>
 
                    <!-- Message -->
                    <tr>
                        <td style="padding: 0 30px 20px 30px; color: #555; font-size: 16px; line-height: 1.6;">
                            <p>
                                A reimbursement submitted by <strong>{{ $reimbursement->name }}</strong> has been <strong style="color: #d9534f;">rejected</strong>.
                            </p>
                            <p>
                                Please review the details and take necessary actions if required.
                            </p>
                        </td>
                    </tr>
 
                    <!-- Footer -->
                    <tr>
                        <td style="padding: 20px 30px; text-align: center; color: #888; font-size: 14px;">
                            Thanks,<br>
                            <strong>{{ config('app.name') }}</strong>
                        </td>
                    </tr>
 
                </table>
            </td>
        </tr>
    </table>
 
</body>
</html>