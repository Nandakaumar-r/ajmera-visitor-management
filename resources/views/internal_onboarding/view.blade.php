...
<div class="container mt-5">
    <h2 class="mb-4 text-center fw-bold" style="margin-top: 90px;">Candidate Full Profile</h2>

    <div class="card shadow p-4">
        {{-- Section 1: Internal ORF Details --}}
        <h4 class="mb-3 text-primary">Internal ORF Details</h4>
        @if($orfCreation)
        <div class="row mb-4">
        ...
        @else
        <p class="text-muted mb-4">No ORF creation details found for this candidate.</p>
        @endif

        <hr>

        {{-- Section 2: Candidate Basic Details --}}
        <h4 class="mb-3 text-primary" style="margin-top: 20px;">Candidate Personal Details</h4>
        <div class="row mb-4">
        ...
        {{-- Section 3: Documents --}}
        <h4 class="mb-3 text-primary" style="margin-top: 20px;">Uploaded Documents</h4>
        <div class="row">
            @php
            $fileGroups = [
            'resume' => 'Resume',
        ...
        <h4 class="mb-3 text-primary" style="margin-top: 20px;">Approval Status</h4>
        <div class="alert 
    @if($orf->status === 'approved') alert-success 
    @elseif($orf->status === 'rejected') alert-danger 
    @elseif($orf->status === 'offered') alert-success 
    @elseif($orf->status === 'cancelled') alert-warning 
    @else alert-secondary 
    @endif
">
            @switch($orf->status)
            @case('Approved')
            Candidate has been <strong>approved</strong> by all reviewers.
            @break

            @case('Rejected')
            Candidate has been <strong>rejected</strong>.
            <br><strong>Reason:</strong> {{ $orf->remarks }}
            @break

            @case('Offered')
            Candidate has been <strong>offered</strong> a position by HR.
            @break

            @case('Cancelled')
            The process has been <strong>cancelled</strong> by HR.
            @break

            @default
            Approval process is <strong>pending</strong>.
            @endswitch
        </div>
